...
var fetchKlinesMultiTFTool = (0, import_tools.createTool)({
  ...
  inputSchema: import_zod.z.object({
    pair: import_zod.z.string().describe('Forex pair, e.g. "EUR/USD"'),
    timeframes: import_zod.z.array(import_zod.z.enum(["15min", "1h", "4h", "1day"])).default(["1h", "4h", "1day"]).describe("Timeframes to fetch"),
    outputsize: import_zod.z.number().int().min(50).max(500).default(200)
  }),
  outputSchema: import_zod.z.object({
    pair: import_zod.z.string(),
    snapshots: import_zod.z.record(
      import_zod.z.string(),
      import_zod.z.object({
        ema20: import_zod.z.number(),
        ema50: import_zod.z.number(),
        ema200: import_zod.z.number(),
        rsi: import_zod.z.number(),
        atr: import_zod.z.number(),
        bbWidthPct: import_zod.z.number(),
        slope20Pct: import_zod.z.number(),
        trend: import_zod.z.enum(["UP", "DOWN", "RANGE"]),
        currentPrice: import_zod.z.number(),
        candlesCount: import_zod.z.number()
      })
    )
  }),
...
var runMicroBacktestTool = (0, import_tools2.createTool)({
  ...
  inputSchema: import_zod2.z.object({
    ...
    direction: import_zod2.z.enum(["BUY", "SELL"]),
    timeframe: import_zod2.z.enum(["15min", "1h", "4h", "1day"]).default("1h"),
    slMultiplier: import_zod2.z.number().default(1.5).describe("SL = entry - slMultiplier * ATR (BUY)"),
    tpMultiplier: import_zod2.z.number().default(3).describe("TP = entry + tpMultiplier * ATR (BUY)"),
    lookback: import_zod2.z.number().int().min(100).max(500).default(300).describe("Bars to scan"),
    forwardBars: import_zod2.z.number().int().min(6).max(100).default(24).describe("Bars to wait for SL/TP")
  }),
  outputSchema: import_zod2.z.object({
    winRate: import_zod2.z.number().describe("Percentage 0-100"),
    avgR: import_zod2.z.number().describe("Average R-multiple realized (positive = profitable)"),
    sampleSize: import_zod2.z.number(),
    regimeMatch: import_zod2.z.boolean().describe("True if current ATR is within 30-70 percentile of historical setups ATR"),
    setupsScanned: import_zod2.z.number()
  }),
  execute: async ({ context }) => {
...
var fetchSessionContextTool = (0, import_tools3.createTool)({
  ...
  outputSchema: import_zod3.z.object({
    pair: import_zod3.z.string(),
    activeSessions: import_zod3.z.array(import_zod3.z.enum(["TOKYO", "LONDON", "NEW_YORK"])),
    isHighLiquidity: import_zod3.z.boolean().describe("True during London-NY overlap"),
    nextSession: import_zod3.z.string(),
...
var SignalDirectionSchema = import_zod4.z.enum(["BUY", "SELL", "HOLD"]);
var SignalSchema = import_zod4.z.object({
  id: import_zod4.z.string(),
  instrument: import_zod4.z.string(),
  category: import_zod4.z.literal("FOREX"),
  tier: import_zod4.z.literal("PRO"),
  direction: SignalDirectionSchema,
  // From Risk Agent (real numbers, not arbitrary multiples)
  entry: import_zod4.z.number(),
  target: import_zod4.z.number(),
  stop: import_zod4.z.number(),
  takeProfits: import_zod4.z.array(import_zod4.z.number()).length(3),
  // TP1, TP2, TP3
  confidence: import_zod4.z.number().min(0).max(100),
  vitality: import_zod4.z.number().min(0).max(100),
  timestamp: import_zod4.z.number(),
  spread: import_zod4.z.number(),
  // 5 dots for SignalCard.tsx (L0=strategies, L1=trend, L2=meanrev, L3=breakout, L4=macro)
  layers: import_zod4.z.array(import_zod4.z.boolean()).length(5),
  layerConfidences: import_zod4.z.array(import_zod4.z.number()).length(5),
  status: import_zod4.z.enum(["active", "expired", "closing_soon"]),
  sourceLayers: import_zod4.z.array(import_zod4.z.string()),
  // PRO-only metadata
  thesis: import_zod4.z.string(),
  mainRisk: import_zod4.z.string(),
  positionSizePct: import_zod4.z.number(),
  riskRewardTp1: import_zod4.z.number(),
  expectedHoldMinutes: import_zod4.z.number(),
  backtestWinRate: import_zod4.z.number(),
  backtestSampleSize: import_zod4.z.number()
});
var CandidateSchema = import_zod4.z.object({
  pair: import_zod4.z.string(),
  // e.g. "EUR/USD"
  direction: import_zod4.z.enum(["BUY", "SELL"]),
  l0Confidence: import_zod4.z.number().min(0).max(100),
  triggeredStrategies: import_zod4.z.array(import_zod4.z.string()),
  currentPrice: import_zod4.z.number()
});
var CouncilRequestSchema = import_zod4.z.object({
  candidates: import_zod4.z.array(CandidateSchema).min(1).max(5)
});
var StrategyVerdictSchema = import_zod4.z.object({
  verdict: import_zod4.z.enum(["CONFIRM", "REJECT", "SKIP"]),
  confidence: import_zod4.z.number().min(0).max(100),
  reasoning: import_zod4.z.string().max(300),
  htfAligned: import_zod4.z.boolean().optional(),
  trendStrength: import_zod4.z.enum(["STRONG", "MODERATE", "WEAK"]).optional(),
  extremeLevel: import_zod4.z.enum(["EXTREME", "ELEVATED", "NEUTRAL"]).optional(),
  divergenceDetected: import_zod4.z.boolean().optional(),
  breakoutType: import_zod4.z.enum(["RANGE", "SQUEEZE", "TREND_CONTINUATION", "NONE"]).optional(),
  followThroughLikely: import_zod4.z.boolean().optional(),
  backtest: import_zod4.z.object({
    winRate: import_zod4.z.number(),
    sampleSize: import_zod4.z.number()
  }).optional()
});
var MacroVerdictSchema = import_zod4.z.object({
  verdict: import_zod4.z.enum(["CONFIRM", "REJECT", "NEUTRAL"]),
  confidence: import_zod4.z.number().min(0).max(100),
  keyEventNext12h: import_zod4.z.string().nullable(),
  fundamentalBias: import_zod4.z.enum(["BULL", "BEAR", "NEUTRAL"]),
  newsSummary: import_zod4.z.string().max(400)
});
var RiskOutputSchema = import_zod4.z.object({
  entryZone: import_zod4.z.object({ low: import_zod4.z.number(), high: import_zod4.z.number() }),
  stopLoss: import_zod4.z.number(),
  takeProfit1: import_zod4.z.number(),
  takeProfit2: import_zod4.z.number(),
  takeProfit3: import_zod4.z.number(),
  positionSizePct: import_zod4.z.number().min(0).max(3),
  riskRewardTp1: import_zod4.z.number(),
  consensusStrength: import_zod4.z.enum(["STRONG", "MIXED", "WEAK"])
});
var JudgeVerdictSchema = import_zod4.z.object({
  decision: import_zod4.z.enum(["PUBLISH", "HOLD", "DROP"]),
  finalDirection: import_zod4.z.enum(["BUY", "SELL", "HOLD"]),
  finalConfidence: import_zod4.z.number().min(0).max(100),
  keyThesis: import_zod4.z.string().max(500),
  mainRisk: import_zod4.z.string().max(300),
  expectedHoldMinutes: import_zod4.z.number().int().min(15).max(2880)
});
...
// src/inngest/councilFn.ts
var runCouncilFn = inngest.createFunction(
  {
    id: "run-council",
    name: "Run Strategy Council",
    concurrency: { limit: 5 },
    // max 5 candidates in parallel
    retries: 1
  },
  { event: "council/run" },
  async ({ event, step }) => {
    ...
    const results = await Promise.all(
      candidates.map(
        (c) => step.run(`council-${c.pair.replace("/", "_")}`, () => runCouncilForCandidate(c))
      )
    );
    const published = [];
...
var scheduledCouncilFn = inngest.createFunction(
